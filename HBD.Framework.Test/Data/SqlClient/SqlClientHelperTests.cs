...
namespace HBD.Framework.Test.Data.SqlClient
{
    [TestClass()]
    public class SqlClientHelperTests
    {
        ...
        //    {
        //        ShimDataClientHelper.AllInstances.ExecuteReaderStringIDictionaryOfStringObject
        //            = (helper, q, dic) =>
        //            {
                ...
                System.Data.SqlClient.Fakes.ShimSqlCommand.AllInstances.ExecuteDbDataReaderCommandBehavior =
                    (s, c) =>
                    {
                        ...
                        if (s.CommandText.ContainsIgnoreCase("SELECT MaxValue"))
                            return new CsvAdapter("TestData\\Northwind_SchemaInfo_MaxValues.csv").Read().CreateDataReader();
            ...
            using (ShimsContext.Create())
            {
                ...
                System.Data.SqlClient.Fakes.ShimSqlCommand.AllInstances.ExecuteDbDataReaderCommandBehavior =
                    (s, c) =>
                    {
                        ...
                        using (var ex = new ExcelAdapter("TestData\\DataBaseInfo.xlsx"))
                        {
                            var dt = ex.ReadData("DataBaseInfo").ToDataTable(true);
        ...
        public void GetSchemaInfoTest()
        {
            ...
            Assert.IsTrue(columns.Any(c => c.MaxPrimaryKeyValue.IsNotNullOrEmpty()));
        ...
        public void SchemaInfo_Dependence_Test()
        {
            ...
            Assert.IsTrue(schema.Tables["CustomerCustomerDemo"].DependenceIndex > schema.Tables["CustomerDemographics"].DependenceIndex
                && schema.Tables["CustomerCustomerDemo"].DependenceIndex > schema.Tables["Customers"].DependenceIndex);

            Assert.AreEqual(schema.Tables["Customers"].DependenceIndex, 0);
            Assert.AreEqual(schema.Tables["Employees"].DependenceIndex, 0);

            Assert.IsTrue(schema.Tables["EmployeeTerritories"].DependenceIndex > schema.Tables["Employees"].DependenceIndex
                && schema.Tables["EmployeeTerritories"].DependenceIndex > schema.Tables["Territories"].DependenceIndex);
        ...
        public void BuildCommandTest()
        {
            ...
            {
                var priObj = new PrivateObject(conn);
                Assert.IsNotNull(priObj.Invoke("CreateCommand", "Select * from A", new Dictionary<string, object> { { "@123", null } }));
            }
        }
