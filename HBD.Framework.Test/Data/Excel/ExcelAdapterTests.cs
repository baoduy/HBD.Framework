...
namespace HBD.Framework.Data.Excel.Tests
{
    ...
    public class ExcelHelperTests
    {
        ...
        public void GetColumnLabel_Exception()
        {
            var priType = new PrivateType(typeof(ExcelAdapter));
            priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, -1);
        }
        ...
        public void GetColumnLabel_Test()
        {
            ...
            Assert.AreEqual("A", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 0));
            Assert.AreEqual("B", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 1));
            Assert.AreEqual("Z", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 25));

            Assert.AreEqual("AA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26));
            Assert.AreEqual("BA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 52));
            Assert.AreEqual("ZA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26));
            Assert.AreEqual("ZZ", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26 + 25));

            Assert.AreEqual("AAA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 27));
            Assert.AreEqual("AAZ", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 27 + 25));
            Assert.AreEqual("ALA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 38));
            Assert.AreEqual("YZA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26 * 26));
            Assert.AreEqual("YZZ", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26 * 26 + 25));

            Assert.AreEqual("ZAA", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26 * 26 + 26));
            Assert.AreEqual("ZZZ", (string)priType.InvokeStatic("GetColumnLabel", BindingFlags.NonPublic, 26 * 26 * 27 + 25));
        }
        ...
        {
            var priType = new PrivateType(typeof(ExcelAdapter));
            priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "");
        }

        [TestMethod()]
        [TestCategory("Fw.Data.ExcelAdapter")]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void GetColumnIndex_ArgumentOutOfRangeException()
        {
            var priType = new PrivateType(typeof(ExcelAdapter));
            priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "A123Z");
        }
        ...
        public void GetColumnIndex_Test()
        {
            ...
            Assert.AreEqual(0, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "A"));
            Assert.AreEqual(1, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "B"));
            Assert.AreEqual(25, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "Z"));

            Assert.AreEqual(26, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "AA"));
            Assert.AreEqual(52, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "BA"));
            Assert.AreEqual(26 * 26, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "ZA"));
            Assert.AreEqual(26 * 26 + 25, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "ZZ"));

            Assert.AreEqual(26 * 27, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "AAA"));
            Assert.AreEqual(26 * 27 + 25, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "AAZ"));
            Assert.AreEqual(26 * 38, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "ALA"));
            Assert.AreEqual(26 * 26 * 26, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "YZA"));
            Assert.AreEqual(26 * 26 * 26 + 25, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "YZZ"));

            Assert.AreEqual(26 * 26 * 26 + 26, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "ZAA"));
            Assert.AreEqual(26 * 26 * 27 + 25, (int)priType.InvokeStatic("GetColumnIndex", BindingFlags.NonPublic, "ZZZ"));
        }
        ...
        public void CreateNew_NoSheet_Test()
        {
            ...
            using (var con = new ExcelAdapter(fileName, op => { op.AutoCreateNewFile = false; op.AddDefaultSheets = false; }))
            {
                con.Create();
                Assert.IsNotNull(con.SpreadsheetDocument);
            }
        ...
        public void AddNewSheet_WithDataTable()
        {
            ...
            {
                var name = con.AddNewSheet(data);
                Assert.IsTrue(con.SheetNames.Length > 3);

                var sheetData = con[name].ToDataTable(true);
                Assert.IsNotNull(sheetData);
                Assert.IsTrue(sheetData.Rows.Count == data.Rows.Count);
                Assert.IsTrue(sheetData.Columns.Count == data.Columns.Count);
            }
        }
        ...
        public void CreateNew_1Sheet_Test()
        {
            ...
            using (var con = new ExcelAdapter(fileName, op => { op.OpenMode = OpenMode.Editable; op.AutoCreateNewFile = false; op.AddDefaultSheets = false; }))
            {
                con.Create();
                con.AddNewSheet();
        ...
        {
            using (var con = new ExcelAdapter("TestData\\2015 Weekly Calendar.xlsx"))
            using (var table = con[0].ToDataTable(false))
            {
                //Validate Data
        ...
        {
            using (var con = new ExcelAdapter("TestData\\2015 Weekly Calendar.xlsx"))
            using (var tb = con["2015 Weekly Calendar"].ToDataTable())
            {
                //Validate Data
        ...
        private static void ValidateCalendarData(System.Data.DataTable data)
        {
            ...
            Assert.AreEqual(data.Rows[1]["B"] as string, "This Excel calendar is blank & designed for easy use as a planner.  Courtesy of WinCalendar");
        ...
        public void ReadDataFromEmptyHeader_Test()
        {
            using (var con = new ExcelAdapter("TestData\\The Empty SheetData.xlsx", op => op.AutoCreateNewFile = true))
            {
                for (var i = 0; i < data.Columns.Count; i++)
                    data.Rows[0][i] = string.Empty;

                var name = con.AddNewSheet(data, true);
                var dt = con[name].ToDataTable(true);
                Assert.AreEqual(data.Rows.Count - 1, dt.Rows.Count);
            }
        }
        ...
        public void WriteDataToSheetData_WithNullData_ReturnNull_Test()
        {
            ...
            {
                var priObj = new PrivateObject(con);
                Assert.IsNull(priObj.Invoke("WriteDataToSheetData", (SheetData)null, data, true));
