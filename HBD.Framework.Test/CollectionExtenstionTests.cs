...
namespace HBD.Framework.Test.Extensions
{
    ...
    public class CollectionExtenstionTests
    {
        ...
        [TestMethod()]
        [TestCategory("Fw.Extensions")]
        public void IsNullOrEmptyDicTest()
        {
            IDictionary<string, TestItem> list = null;
            Assert.IsTrue(list.IsEmpty());

            list = new Dictionary<string, TestItem>();
            Assert.IsTrue(list.IsEmpty());

            list.Add("A", new TestItem());
            Assert.IsFalse(list.IsEmpty());
        }
        ...
        public void HasDuplicatedItemTest()
        {
            ...
            Assert.IsFalse(list.HasDuplicatedItem(t => t.Name));
            Assert.IsFalse(list.HasDuplicatedItem(t => t.Id));

            list = new List<TestItem> { new TestItem { Name = "A" }, new TestItem { Name = "A" } };
            Assert.IsTrue(list.HasDuplicatedItem(t => t.Name));
            Assert.IsFalse(list.HasDuplicatedItem(null));

            list = new List<TestItem> { new TestItem { Name = "A" } };
            Assert.IsFalse(list.HasDuplicatedItem(t => t.Name));

            list = new List<TestItem> { new TestItem { Id = 1 }, new TestItem { Id = 1 } };
            Assert.IsTrue(list.HasDuplicatedItem(t => t.Id));
            Assert.IsFalse(list.HasDuplicatedItem(null));
        ...
        public void MergeFrom_ListToList1_Test()
        {
            ...
            Assert.IsFalse(list1.HasDuplicatedItem(t => t.Name));
        }
        ...
        public void MergeFrom_ListToList2_Test()
        {
            ...
            var list2 = new List<TestItem> { new TestItem { Id = 2, Name = "A" }, new TestItem { Id = 3, Name = "B" }, new TestItem { Id = 3, Name = "C" } };
            ...
            Assert.IsFalse(list1.HasDuplicatedItem(t => t.Id));
        }
