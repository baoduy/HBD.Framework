...
namespace HBD.Framework.Data.Csv
{
    public class CsvAdapter : DataFileAdapterBase
    {
        ...
        {
            if (table == null || !File.Exists(this.DocumentFile)) return;

            var op = new ReadCsvOption();
            options?.Invoke(op);

            table.TableName = Path.GetFileNameWithoutExtension(this.DocumentFile);
            table.Columns.Clear();
            table.Rows.Clear();
            ...
            using (var csvReader = new TextFieldParser(DocumentFile))
            {
                ...
                if (op.FieldWidths.IsNotNullOrEmpty())
        ...
        public virtual void Write(IGetSetterCollection data, Action<WriteCsvOption> options = null)
        {
            ...
            using (var writer = new StreamWriter(this.DocumentFile))
            {
                if (!op.IgnoreHeader)
        ...
        public override void WriteData(DataSet data, bool ignoreHeader = true)
        {
            ...
            this.Write(new DataTableGetSetterCollection(data.Tables[0]), op => op.IgnoreHeader = ignoreHeader);
        }
    }
}