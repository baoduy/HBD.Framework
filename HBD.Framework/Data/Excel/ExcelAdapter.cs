...
namespace HBD.Framework.Data.Excel
{
    ...
    public class ExcelAdapter : DataFileAdapterBase
    {
        ...
        //protected internal virtual DataTable ReadDataFromWorksheetPart(WorksheetPart worksheetPart)
        //    => ReadDataFromSheetData(worksheetPart.GetSheetData());

        //protected internal virtual DataTable ReadDataFromSheetData(SheetData sheetData, bool firstRowIsHeader = false)
        //{
        //    if (sheetData == null) return null;
        //    var data = new DataTable { TableName = sheetData.LocalName };

        //    foreach (var cells in sheetData.Elements<Row>().Select(row => row.Descendants<Cell>().ToList()))
        //    {
        //        if (firstRowIsHeader && data.Columns.Count <= 0)
        //        {
        //            foreach (var cell in cells)
        //            {
        //                var val = cell.GetValue(SpreadsheetDocument.WorkbookPart);
        //                var strVal = val?.ToString() ?? string.Empty;
        //                if (strVal.IsNullOrEmpty())
        //                    strVal = GetColumnLabel(data.Columns.Count);
        //                data.Columns.Add(strVal, typeof(object));
        //            }
        //            continue;
        //        }

        //        var max = cells.Max(c => GetCellIndex(c.CellReference).ColumnIndex) + 1;
        //        if (max > data.Columns.Count)
        //            data.AddMoreColumns(max, ColumnNamingType.ExcelType);

        //        var r = data.NewRow();
        //        foreach (var cell in cells)
        //        {
        //            var val = cell.GetValue(SpreadsheetDocument.WorkbookPart);
        //            var cellIndex = GetCellIndex(cell.CellReference);
        //            r[(int)cellIndex.ColumnIndex] = val;
        //        }
        ...
        protected virtual Row WriteDataToExcelRow(DataRow row, int rowIndex)
        {
            ...
            {
                var obj = row.ItemArray[c].IsNullOrEmpty() ? string.Empty : row.ItemArray[c];
        ...
        protected virtual void Create(SpreadsheetDocumentType documentType)
        {
            ...
            this.Options.OpenMode = OpenMode.Editable;
        ...
        public override void Dispose() => SpreadsheetDocument?.Dispose();
        ...
        {
            columnIndex.MustGreaterThan(-1, nameof(columnIndex));
        ...
        {
            Guard.ArgumentIsNotNull(columnLabel, nameof(columnLabel));
            if (columnLabel.Any(c => (c < 'A') || (c > 'Z')))
                throw new ArgumentOutOfRangeException(nameof(columnLabel));

            var colIndex = 0;
            for (int i = 0, pow = columnLabel.Count() - 1; i < columnLabel.Count(); ++i, --pow)
            {
                var cVal = Convert.ToInt32(columnLabel[i]) - 64;
        ...
        public static CellIndex GetCellIndex(string cellReference)
        {
            ...
            try
            {
                ...
                var colIndex = string.IsNullOrEmpty(splits[0]) ? -1 : GetColumnIndex(splits[0]);
        ...
        protected static bool IsExcelFile(string documentFile)
        {
            ...
            return !ext.IsNullOrEmpty() && SupportedExcelExtensions.ContainsIgnoreCase(ext.Replace(".", string.Empty));
        }
        ...
        public virtual void WriteData(DataTable data, bool ignoreHeader = true)
        {
            ...
            if (data.TableName.IsNullOrEmpty() || this.SheetNames.All(n => !n.Name.EqualsIgnoreCase(data.TableName)))
            {
                name = emptySheet.IsNullOrEmpty() ? null : emptySheet;
            }
            else name = this.SheetNames.FirstOrDefault(n => n.Name.EqualsIgnoreCase(data.TableName))?.Name;

            if (name.IsNullOrEmpty())
                this.AddNewSheet(data, ignoreHeader);
            else this.WriteDataToWorksheetPart(GetWorksheetByName(emptySheet), data, ignoreHeader);
        }

        private string GetFirstEmptySheet() => this.SheetNames.Where(n =>
        {
        ...
        /// <summary>
        /// Add New Fornt Size, Color and Font Name to SpreadsheetDocument.
        /// If Font Name is empty the Default Font Name will be picked up.
        /// </summary>
        /// <param name="fontSize"></param>
        /// <param name="color"></param>
        /// <param name="fontName"></param>
        /// <returns></returns>
        //public virtual uint AddNewFont(double fontSize, System.Drawing.Color color, string fontName = null)
        //{
        ...
        //    var workbookStylesPart = this.SpreadsheetDocument.WorkbookPart.GetPartById(DefaultWorkbookStylesPartID) as WorkbookStylesPart;
        //    if (workbookStylesPart == null)
        //        workbookStylesPart = AddDefaultWorkbookStylesPart(this.SpreadsheetDocument);

        //    var rgb = color.ToHexBinary();
        ...
        //public virtual uint AddNewFill(System.Drawing.Color color)
        //{
        ...
        //public virtual uint AddNewCellFormat(SpreadsheetDocument spreadsheet, CellStyle cellStyle)
        //{
        //    if (cellStyle == null || cellStyle.IsEmpty())
        //        return Convert.ToUInt32((int)DefaultCellStyle.Default);

        //    var fillID = AddNewFill(cellStyle.BackgroundColor);
        //    var fontID = AddNewFont(DefaultFontSize, cellStyle.ForeColor);

        //    var workbookStylesPart = spreadsheet.WorkbookPart.GetPartById(DefaultWorkbookStylesPartID) as WorkbookStylesPart;
        ...
        //public virtual void FillToWorksheet(WorksheetPart worksheetPart, DataTable data, CellStyleCollection styles = null)
        //{
        ...
        //    int numberOfColumns = datColumns.Count;
        //    bool[] IsNumericColumn = new bool[numberOfColumns];

        //    string[] excelColumnNames = new string[numberOfColumns];
        //    for (int n = 0; n < numberOfColumns; n++)
        //        excelColumnNames[n] = GetColumnLabel(n);
        ...
        //    sheetDatAppend(headerRow);

        //    //Add Columns to First Row of Excel file
        //    for (int colInx = 0; colInx < numberOfColumns; colInx++)
        //    {
        //        DataColumn col = datColumns[colInx];
        //        var cell = CreateTextCell(excelColumnNames[colInx] + "1", col.ColumnName);
        //        cell.StyleIndex = (int)DefaultCellStyle.Bold;
        //        headerRow.Append(cell);
        //        IsNumericColumn[colInx] = col.DataType.IsNumericType();

        //        ApplyCellStype(cell, styles, Convert.ToInt32(rowIndex), colInx);
        //    }
        ...
        //    foreach (DataRow dr in datRows)
        //    {
        //        // ...create a new row, and append a set of this row's data to it.
        //        ++rowIndex;
        //        var newExcelRow = new Row { RowIndex = rowIndex };  // add a row at the top of spreadsheet
        //        sheetDatAppend(newExcelRow);
        ...
        //protected static Stylesheet GenerateDefaultStyleSheet()
        //{
        ...
        //                new Color()
        //                {
        //                    Rgb = System.Drawing.Color.Black.ToHexBinary()
        //                },
        //                new FontName() { Val = DefaultFontName }),
        ...
        //                    Rgb = System.Drawing.Color.Black.ToHexBinary()
        //                },
        //                new FontName() { Val = TimesNewRomanFontName })
        //        ),
        //        new Fills(
        ...
        //                    new ForegroundColor()
        //                    {
        //                        Rgb = System.Drawing.Color.Yellow.ToHexBinary()
        //                    }
        //                )
        //                { PatternType = PatternValues.Solid }),
        ...
        //                    new ForegroundColor()
        //                    {
        //                        Rgb = System.Drawing.Color.Red.ToHexBinary()
        //                    }
        //                )
        //                { PatternType = PatternValues.Solid }),
        ...
        //                    new ForegroundColor()
        //                    {
        //                        Rgb = System.Drawing.Color.Green.ToHexBinary()
        //                    }
        //                )
        //                { PatternType = PatternValues.Solid })
        //        ),
        ...
        //protected static WorkbookStylesPart AddDefaultWorkbookStylesPart(SpreadsheetDocument spreadsheet)
        //{
        //    var workbookStylesPart = spreadsheet.WorkbookPart.AddNewPart<WorkbookStylesPart>(DefaultWorkbookStylesPartID);
