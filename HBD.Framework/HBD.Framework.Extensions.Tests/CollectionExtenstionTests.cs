...
namespace HBD.Framework.Extensions.Tests
{
    ...
    public class CollectionExtenstionTests
    {
        ...
        [TestMethod]
        [TestCategory("Fw.Extensions")]
        public void Dictionary_AddRange_Array_Test()
        {
        ...
        public void HasDuplicatedItemTest()
        {
            ...
            Assert.IsFalse(list.HasDuplicatedItems(t => t.Name));
            Assert.IsFalse(list.HasDuplicatedItems(t => t.Id));

            list = new List<TestItem> {new TestItem {Name = "A"}, new TestItem {Name = "A"}};
            Assert.IsTrue(list.HasDuplicatedItems(t => t.Name));
            Assert.IsFalse(list.HasDuplicatedItems(null));

            list = new List<TestItem> {new TestItem {Name = "A"}};
            Assert.IsFalse(list.HasDuplicatedItems(t => t.Name));

            list = new List<TestItem> {new TestItem {Id = 1}, new TestItem {Id = 1}};
            Assert.IsTrue(list.HasDuplicatedItems(t => t.Id));
            Assert.IsFalse(list.HasDuplicatedItems(null));
        ...
        public void MergeFrom_DicToList_Test()
        {
            ...
            Assert.IsTrue(list1.Count == 2);
            Assert.IsFalse(list1.HasDuplicatedItems(t => t.Name));
        }
        ...
        public void MergeFrom_ListToList2_Test()
        {
            ...
            list1.MergeFrom(list2, t => t.Id);
            Assert.IsTrue(list1.Count == 3);
            Assert.IsFalse(list1.HasDuplicatedItems(t => t.Id));
        }
        ...
        {
            IList<TestItem> list = new List<TestItem>
            {
            ...
            var dic1 = Enumerable.ToDictionary(list, t => t.Name, t => t);
            Assert.IsTrue(dic1.Count == 3);

            var dic2 = Enumerable.ToDictionary(list, t => t.Id, t => t);
